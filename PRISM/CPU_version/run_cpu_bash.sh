...
# CPU baseline runner for artifact (summary only)
...
OUTPUT_FILE="${OUTPUT_FILE:-${SCRIPT_DIR}/results.csv}"
...
if [[ ! -x "${BUILD_DIR}/epicap" ]]; then
    print_error "CPU binary not found. Run 'make' in ${SCRIPT_DIR} first."
    exit 1
fi
